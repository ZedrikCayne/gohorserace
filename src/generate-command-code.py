    ...
    def struct_code(self):
        def _flags_code():
            ...
            for flag in self.spec.get("flags", []):
                s += "Command.KeySpecFlags.CMD_KEY_%s," % flag
        ...
        return "new Command.RedisKeySpec().set_notes(%s).set_flags(%s)%s%s" % (
...
class Argument(object):
    ...
    def struct_code(self):
        ...
        def _flags_code():
            ...
            if self.desc.get("optional", False):
                any = True
                s += "Command.CommandArgFlag.CMD_ARG_OPTIONAL,"
            if self.desc.get("multiple", False):
                any = True
                s += "Command.CommandArgFlag.CMD_ARG_MULTIPLE,"
            if self.desc.get("multiple_token", False):
                any = True
                assert self.desc.get("multiple", False)  # Sanity
                s += "Command.CommandArgFlag.CMD_ARG_MULTIPLE_TOKEN,"
            if not any:
                s += "Command.CommandArgFlag.CMD_ARG_NONE,"
            return s[:-1]+")"

        s = "Command.Argument.MAKE_ARG(\"%s\",Command.RedisCommandArgType.%s,%d,%s,%s,%s,%s,%d,%s)" % (
            self.name,
            ARG_TYPES[self.type],
        ...
        if "display" in self.desc:
            s += ".set_display_text(\"%s\")" % self.desc["display"].lower()
        if self.subargs:
            s += ".set_subargs(%s)" % self.subarg_table_name()
    ...
    def write_internal_structs(self, f):
        if self.subargs:
            ...
            f.write("public static List<Command.Argument> %s = List.of(\n" % self.subarg_table_name())
            prefix=""
            for subarg in self.subargs:
                f.write("%s%s\n" % (prefix, subarg.struct_code()))
                prefix=","
            f.write(");\n\n")
...
class Command(object):
    ...
    def history_code(self):
        ...
        for tupl in self.desc["history"]:
            s += "new Command.CommandHistory(\"%s\",\"%s\"),\n" % (tupl[0], tupl[1])
        return s[:-2]
    ...
    def struct_code(self):
        ...
        def _flags_code():
            ...
            for flag in self.desc.get("command_flags", []):
                any = True
                s += "Command.CommandFlags.CMD_%s," % flag
            return s[:-1]+")" if any else "EnumSet.noneOf(Command.CommandFlags.class)"
        ...
        def _acl_categories_code():
            ...
            for cat in self.desc.get("acl_categories", []):
                any = True
                s += "Command.AclCategoryFlags.ACL_CATEGORY_%s," % cat
            return s[:-1]+")" if any else "EnumSet.noneOf(Command.AclCategoryFlags.class)"
        ...
        def _doc_flags_code():
            ...
            for flag in self.desc.get("doc_flags", []):
                any = True
                s += "Command.CommandDocFlags.CMD_DOC_%s," % flag
            if not any:
                s += "Command.CommandDocFlags.CMD_DOC_NONE,"
            return s[:-1]+")"

        s = "Command.MAKE_CMD(\"%s\",%s,%s,%s,%s,%s,%s,%s,%s,%s,%d,%s,%d,%s,%d,%s,%s,%s,%d,%s,%d)" % (
            self.name.lower(),
            get_optional_desc_string(self.desc, "summary"),
            ...
            get_optional_desc_string(self.desc, "deprecated_since"),
            "\"%s\"" % self.group,
            "Command.CommandGroup."+GROUPS[self.group],
            self.history_table_name(),
            self.num_history(),
            self.tips_table_name(),
            self.num_tips(),
            "\"%s\"" % self.desc.get("function", "null"),
            self.desc["arity"],
            _flags_code(),
            _acl_categories_code(),
            self.key_specs_table_name(),
            len(self.key_specs),
            "\"%s\"" % self.desc.get("get_keys_function", "null"),
            len(self.args),
        )
    ...
    def write_internal_structs(self, f):
        if self.subcommands:
            ...
            f.write("public static List<Command> %s = List.of(\n" % self.subcommand_table_name())
            prefix = ""
            for subcommand in subcommand_list:
                f.write("%s%s\n" % (prefix,subcommand.struct_code()))
                prefix = ","
            f.write(");\n\n")
        ...
        if code:
            f.write("public static List<Command.CommandHistory>%s = List.of(\n" % self.history_table_name())
            f.write("%s" % code)
            f.write(");\n")
        else:
            f.write("public static List<Command.CommandHistory>%s = null;\n" % self.history_table_name())

        f.write("/* %s tips */\n" % self.fullname())
        code = self.tips_code()
        if code:
            f.write("public static List<String> %s = List.of(\n" % self.tips_table_name())
            f.write("%s" % code)
            f.write(");\n")
        else:
            f.write("public static List<String> %s = null;\n" % self.tips_table_name())

        f.write("/* %s key specs */\n" % self.fullname())
        code = self.key_specs_code()
        if code:
            f.write("public static List<Command.RedisKeySpec> %s = List.of(\n" % (self.key_specs_table_name()))
            f.write("%s\n" % code)
            f.write(");\n")
        else:
            f.write("public static List<Command.RedisKeySpec> %s = null;\n" % self.key_specs_table_name())
...
dstdir = os.path.abspath(os.path.dirname(os.path.abspath(__file__)) + "/../app/src/main/java/com/justaddhippopotamus/myredis/Server/Commands")
dstimpldir = os.path.abspath(os.path.dirname(os.path.abspath(__file__)) + "/../app/src/main/java/com/justaddhippopotamus/myredis/server/commands/impl")
...
commands_filename = "commands_with_reply_schema" if args.with_reply_schema else "ServerCommands"
print("Generating %s.java..." % commands_filename)
with open("%s/%s.java" % (dstdir, commands_filename), "w") as f:
    f.write("/* Automatically generated by %s, do not edit. */\n\n" % os.path.basename(__file__))
    f.write("package com.justaddhippopotamus.myredis.server.commands;\n\n")
    f.write(
"""
...
import com.justaddhippopotamus.myredis.server.commands.impl.*;
import com.justaddhippopotamus.myredis.server.Command;
import com.justaddhippopotamus.myredis.server.ICommandImplementation;
import com.justaddhippopotamus.myredis.server.ICommandKeyParser;
import java.util.*;
...
"""
    ...
    command_functions = set()
    keys_functions = set()
    for command in command_list:
        fname = command.desc.get("function", "null")
        getKeys = command.desc.get("get_keys_function", "null")
        if fname != "null":
            command_functions.add(fname)
        if getKeys != "null":
            keys_functions.add(getKeys)
        if command.subcommands:
            subcommand_list = sorted(command.subcommands, key=lambda cmd: cmd.name)
            for subcommand in subcommand_list:
                fname = subcommand.desc.get("function", "null")
                getKeys = command.desc.get("get_keys_function", "null")
                if fname != "null":
                    command_functions.add(fname)
                if getKeys != "null":
    ...
    cmd_fun_sorted = sorted(command_functions)
    key_fun_sorted = sorted(keys_functions)

    for command in cmd_fun_sorted:
        class_name = "Command" + command[0].upper() + command[1:]
        dstJavaFilename = "%s/%s.java" % (dstimpldir, class_name)
        if not os.path.exists( dstJavaFilename ):
            with open(dstJavaFilename, "w") as g:
                g.write(
"""
package com.justaddhippopotamus.myredis.server.commands.impl;

import com.justaddhippopotamus.myredis.RESP.RESPArray;
import com.justaddhippopotamus.myredis.server.ICommandImplementation;
import com.justaddhippopotamus.myredis.server.Command;
import com.justaddhippopotamus.myredis.server.Client;

/* File just to have a class here for the ServerCommands to import should this
   directory be otherwise empty.
 */
public class %s implements ICommandImplementation {
    @Override
    public void runCommand(RESPArray commands, Client client) {
        Command.BadDefaultCommandImplementation(commands, client);
    }
}
""" % class_name )

    for keyfun in key_fun_sorted:
        class_name = "KeySearch" + keyfun[0].upper() + keyfun[1:]
        dstJavaFilename = "%s/%s.java" % (dstimpldir, class_name)
        if not os.path.exists( dstJavaFilename ):
            with open(dstJavaFilename, "w") as g:
                g.write(
...
import com.justaddhippopotamus.myredis.RESP.RESPArray;
import com.justaddhippopotamus.myredis.server.ICommandKeyParser;
public class %s implements ICommandKeyParser {
    @Override
...
""" % class_name )
    ...
    f.write("public static List<Command> redisCommandTable = List.of(\n")
    curr_group = None
    prefix = ""
    for command in command_list:
