 ...
 */
public class CommandHscanCommand extends ICommandImplementation {
    @Override
    public void runCommand(WorkItem item) {
        ...
        RedisHash rh = null;
        if( cursorId == 0 ) {
            rh = item.getMainStorage().fetchRO(key, RedisHash.class);
            if( rh == null ) {
        ...
        RESPArray ra = rc.scanHash(count,pattern,NOVALUES);
